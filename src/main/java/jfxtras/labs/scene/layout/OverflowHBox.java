...
//    	System.out.println(">> " + getWidth() + " / " + hbox.getWidth());
//    	System.out.println(">>>> " + prefWidth(-1) + " / " + hbox.prefWidth(-1));
    	super.layoutChildren();
    	
    	// should we remove a node?
		while (hbox.getChildren().size() > 0) {
			if (getWidth() <= hbox.prefWidth(-1) + dropDown.prefWidth(-1)) {
		...
		while (vbox.getChildren().size() > 0) {
			...
			double prefWidth = candidateToBeAdded.prefWidth(-1);
			if (getWidth() > hbox.prefWidth(-1) + dropDown.prefWidth(-1) + prefWidth) {
